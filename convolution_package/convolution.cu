...
__global__ void kernel_convolution(float *image,float *kernel,float *result,int input_size_x,int input_size_y,int kernel_size,int input_channel,int output_channel,int strides)
{
	...
	const int Idx=(index/output_size_y)%output_size_x;//?
	const int Idy=index%output_size_y;

	if(index>=output_size_x*output_size_y)
	{
		return;
	}
	...
	for(int dx=0;dx<kernel_size;++dx)
	{
		for(int dy=0;dy<kernel_size;++dy)
		{
			...
			{
				for(int dout=0;dout<output_channel;++dout)
				{
					...
					result[index*output_channel+dout]+=kernel[kernel_pos1+kernel_pos2+dout]*image[((Idx*strides+dx)*input_size_y+Idy*strides+dy)*input_channel+dint];
				}
			}
		}
	}
...
int main()
{
	...
	const dim3 Blocks_Per_Grid(int((Output_Size_X-1)/Threads_Per_Block.x)+1,int((Output_Size_Y-1)/Threads_Per_Block.y)+1);// Need To Dec & Add 1 ?
	...
	for(int i=0;i<Output_Size_X;++i)
	{
		for(int j=0;j<Output_Size_Y;++j)
		{
			...
			{
				//printf("%f  ",CPU_Output[i*Output_Size_Y*Output_Channel+j*Output_Channel+k]);
